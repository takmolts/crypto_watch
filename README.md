...
- 先頭 embed: 地形図PNG + 図の読みどころ (数値から機械生成するので図と必ず一致する)。
  ...
  値動きは PNG には描かず、サイト側で価格ラダーと同じ縦軸に 1h足を並べて水準の横線を重ねる
...
1. **概要** — 主要数値のタイル、発火理由、考察の1章、価格ラダー、地形図。
   ラダーの横には同じ縦軸で 1h足を並べ、壁・支持・磁石・Max Pain・フリップ・ETF水準を横線で重ねる
   (足はブラウザから Binance、だめなら Coinbase の公開APIを直接読む)。
   「最新データを取得」でブラウザから Deribit / Coinbase / Binance を直接読み、考察時点との差を重ねる
2. **分析** — 考察の2〜7章。3章の予定はイベント年表にも起こす
   (元データは考察末尾の ```` ```events ```` ブロック「日付 | 時刻 | 種別 | 予定名 | 一言」と、
   各行の下の「  - 」解説3行 (何のイベントか・なぜ見るか・どちらに効くか)。
   プロンプトで書かせ、`publish_pages.py` が `events` に変換する。無い古い考察は本文の日付から拾う)
