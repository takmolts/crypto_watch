...
- 3章の末尾に、年表を自動で起こすための一覧を次のコードブロックで1つ置く。1行1予定で区切りは「|」。日付は年を補って YYYY-MM-DD、時刻は「HH:MM JST」か不明なら「-」、種別は expiry(オプション満期)/cb(中銀・FOMC)/macro(経済指標)/crypto(暗号資産・規制)/other のどれか。期間ものは開始日で1行にして予定名に期間を書く。本文に書いた予定を漏れなく載せる
\`\`\`events
