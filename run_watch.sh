...
- 3章の各予定は箇条書き1行目を「**M/D (曜) HH:MM JST — 予定名**。効き方…」の形にする (時刻不明なら省略)
- 3章の末尾に、年表を自動で起こすための一覧を次のコードブロックで1つ置く。1行1予定で区切りは「|」。日付は年を補って YYYY-MM-DD、時刻は「HH:MM JST」か不明なら「-」、種別は expiry(オプション満期)/cb(中銀・FOMC)/macro(経済指標)/crypto(暗号資産・規制)/other のどれか。期間ものは開始日で1行にして予定名に期間を書く。本文に書いた予定を漏れなく載せる。各予定の下には、その予定を知らない読者向けの解説を「  - 」で始まる箇条書き3行で添える
\`\`\`events
YYYY-MM-DD | HH:MM JST | macro | 予定名 | 効き方を一言
  - 何の指標・出来事か (発表主体・頻度・何を測るか)
  - 市場がなぜ注目するか
  - 上振れ/下振れ (可決/否決) がどちらにどう効くか
\`\`\`
- 6章の各シナリオは「**シナリオ名 — 確度 NN%**。価格帯と根拠」の形、7章は「**条件の具体値** → 帰結」の形で書く
- 最後に出典URLを並べる"
